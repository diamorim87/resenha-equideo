...
As 16 pranchas PNG desta pasta foram criadas com a ferramenta integrada de geração de imagens. Mostram animais fictícios e têm finalidade didática; não são fotografias de exemplares registrados nem substituem a observação do animal ou a avaliação de um profissional.
...
- **Particularidades da cabeça:** o mesmo cavalo fictício alazão visto de frente, da testa ao focinho, com uma única marca branca no local indicado pelo nome e descrição em `src/components/GuideModal.tsx`. O arquivo `bebe-em-branco.png` foi gerado separadamente para restringir a marca ao lábio.
